...
As discussed in the Section on [Pairwise image registration], the purpose of the rigid LSQ6 transformation is to perform a coarse alignment of the images. In the case of multi-image registration, it isn't obvious what the target of the alignment should be. The consensus average space has not yet been constructed and so obviously cannot be used as the target. In practice, we use what is called an **initial model** (colloquially called "init model"). The initial model is a target brain and serves as a representation of the orientation in which we prefer to perform our analyses.     
